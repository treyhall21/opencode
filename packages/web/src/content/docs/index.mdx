...
1. **Create a plan**
   ...
   For example, a rough request like:

   ```txt frame="none"
   Google Maps/ Apple Maps navigation app/website with cool features so it's better than those maps
   ```

   works better when you spell out the product goals:

   ```txt frame="none"
   Build a responsive navigation web app inspired by Google Maps and Apple Maps.
   Include turn-by-turn directions, traffic-aware rerouting, multi-stop trip planning,
   walking/biking/transit modes, offline saved routes, and live hazard reports.
   Add standout features like AI-powered scenic or safest-route suggestions, collaborative
   trip sharing, and accessibility-first route filters. First propose an MVP plan,
   recommended map/data providers, and the main screens before making code changes.
   ```
